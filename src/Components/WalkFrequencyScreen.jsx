function WalkFrequencyOptionCard({ option, onSelect }) {
  return (
    <button
      type="button"
...
export default function WalkFrequencyScreen({
  options = [],
  onSelect,
  onBack,
}) {
    ...
    <section className="min-h-screen bg-[#f5f5f5] px-2 pb-10 pt-3 md:px-4">
      <div className="w-full">
        <div className="mb-2 flex items-center justify-between">
          ...
          <div className="text-[14px] font-medium text-[#6f6f6f]">10 / 24</div>
        </div>

        <div className="relative h-[5px] w-full overflow-hidden rounded-full bg-[#e6e6e3]">
          <div className="h-full w-[41.67%] rounded-full bg-[#2f7a4d]" />
        </div>
      </div>
