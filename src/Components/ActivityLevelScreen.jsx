function ActivityLevelOptionCard({ option, onSelect }) {
  return (
    <button
      onClick={() => onSelect(option.label)}
      className={`flex min-h-[78px] w-full items-center justify-between rounded-[14px] px-7 transition ${
...
export default function ActivityLevelScreen({ options, onSelect, onBack }) {
  return (
    <section className="min-h-screen bg-[#f3f3f3] px-4 pb-10 pt-4">
      <div className="w-full">
        <div className="mb-2 flex items-center justify-between text-[#a8a8a8]">
          ...
          <div className="text-[14px] text-[#7c7c7c]">6 / 24</div>
        </div>

        <div className="relative h-[6px] w-full overflow-hidden rounded-full bg-[#e7e7e4]">
          <div className="h-full w-[26%] rounded-full bg-[#3b7b50]" />
        </div>
      </div>
